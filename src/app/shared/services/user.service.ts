...
export class UserService {
  ...
  isUserLoggedIn(): Promise<boolean> {
    return new Promise((resolve) => firebase.getCurrentUser()
      .then(user => {
  ...
  getUserUid(): Promise<string> {
    return firebase.getCurrentUser().then((user: any) => {
      return user.uid;
    }).catch((err) => {
  ...
  createNewUserInstance (user: User) {
    return firebase.firestore.collection("users").doc(user.googleUserUid).set(user.toDocEntries())
      .then((docRef: any) => {
